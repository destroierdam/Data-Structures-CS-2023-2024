...
public:
	...
	class Iterator {
	public:
		Iterator(BST* bst, Node* node) :bst(bst) {
			this->pushLeftChildren(node);
		}
		Iterator& operator++() {
			Node* current = this->nodes.top();
			this->nodes.pop();
			this->pushLeftChildren(current->right);
			return *this;
		}

		bool operator!=(const Iterator& other) const {
			if (this->bst != other.bst) {
				return true;
			}
			/*if (this->nodes.empty()) {
			...
			// OR
			*/
			return this->topSafe() != other.topSafe();
		}
